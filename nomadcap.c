...
nomadcap_exit(nomadcap_pack_t *np, int code) {
    if (np) {
        /* Free strings */
        if (np->device) free(np->device);
	if (np->filename) free(np->filename);

        /* Close capture device */
        if (np->p) pcap_close(np->p);
...
void
nomadcap_usage(nomadcap_pack_t *np) {
    /* Banner*/
    printf("%s v%s [%s]\n\n",
	np->pname,
	NOMADCAP_VERSION,
